...
6. **`git stash` é PROIBIDO dentro de uma worktree.** Vale igual para `git
   ...
   Estado temporário vai em **commit na própria branch da worktree** — `wip:`
   serve, é local e reescrevível antes do PR, e guarda staged, não staged e
   arquivo novo por construção, menos o que está no `.gitignore`.
   ...
   **vazio** para arquivo novo não rastreado, de modo que o patch parece salvo e
   ...
   patch, levando junto o arquivo de texto que estava no mesmo patch. Vale em
   qualquer repo com mais de uma worktree ativa, não só no fluxo de ondas.
...
temporário? Commit `wip:` na sua própria branch — é a saída primária, porque
guarda por construção staged, não staged e arquivo novo; não o que está no
`.gitignore`, e `.wave/<ticket>/contract.md` está, então copie o contrato à
parte. Ele é seu, é local, e some com um rebase.
...
`stash`, `pop` e `apply`, o commit `wip:` como saída primária e o patch na
forma completa (`git add -A && git diff --staged --binary`). A **fonte** da
regra é o item 6, o único lugar onde mora o motivo mecânico completo
(`refs/stash` no git dir comum, e os quatro refs que de fato são
