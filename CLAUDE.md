...
- **`scripts/waves/`** — read-only wave pipeline: `tickets-linear.mjs` (Linear project into normalized tickets), `graph.mjs` (`planWaves()`), `pr-state.mjs` (CI state), `fetch-pr-threads.mjs` (PR feedback). See [`docs/waves.md`](docs/waves.md).
