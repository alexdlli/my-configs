...
- **`scripts/waves/`** — read-only wave pipeline: `tickets-linear.mjs` (Linear project into normalized tickets), `tickets-github.mjs` (GitHub Issues into the same shape, via `gh`), `graph.mjs` (`planWaves()`), `pr-state.mjs` (CI state), `fetch-pr-threads.mjs` (PR feedback). See [`docs/waves.md`](docs/waves.md).
