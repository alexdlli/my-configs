...
Um escopo vira tickets, e cada ticket vira o prompt de um agente que nasce sem contexto.
Esta página cobre as duas peças disso que moram no harness: o **contrato de ticket** — o que
um ticket precisa ter para funcionar como prompt — e o **leitor de GitHub Issues**, que
devolve os tickets já normalizados.

Quem executa os tickets é o orquestrador delegando aos especialistas, na mesma resposta.
Não há grafo de dependências nem plano de ondas aqui: o `blockedBy` continua sendo declarado
no ticket (campo 9) e continua saindo do leitor, mas quem lê a ordem é você.
...
Mais dois campos que dizem se um bloqueador conta:

- `statusType` — o `type` do estado no tracker, normalizado para `completed` quando o ticket
  está entregue (no GitHub sai de `CLOSED` + `COMPLETED`). É o sinal confiável de "mergeado";
  o nome do estado é livre e varia por time.
- `external` — `true` quando o ticket não pertence ao escopo lido e só está no registro porque
  alguém depende dele.

O leitor emite ainda um extra próprio, `blockedBySources` — veja
[Fonte GitHub Issues](#fonte-github-issues).
...
`scripts/github/tickets-github.mjs` é o único leitor de tickets, e emite o formato normalizado
acima. Somente leitura. O que ele resolve por conta própria é o recorte, a origem das arestas e
a origem da estimativa.
...
`--state all`, porque bloqueador já fechado é o que libera quem depende dele.
...
em caixa alta no stderr, porque uma leitura do repo todo quando a pessoa queria um milestone é
o tipo de erro que só aparece tarde:
...
`both`). Nenhum consumidor lê esse campo; ele existe para depurar aresta errada.
...
Fechar como *not planned* não é entregar — o dependente continua bloqueado. A terceira linha é
...
mergeado de fechado sem merge, e chutar aí liberaria um dependente que ainda está bloqueado.
...
cair para GraphQL cru paginado: lista de bloqueadores truncada some com dependência real, e um caso de 50+
...
A lógica pura é separada do I/O e testada sozinha: dada a resposta do CLI, qual é o array
normalizado? Nenhum teste chama `gh`.
