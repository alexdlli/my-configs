...
- **A hook is identified by the script it executes under a matcher that covers ours — never by the name in its command.** The installer skips registering a harness hook only when some entry for that event runs the same *absolute* script in executable position (`~/` expanded), with no redirection or chaining, under a matcher that covers the one we declare. Everything else installs ours, which is the safe direction. Three forms used to read as "already registered" and register nothing: a command that merely *mentions* the basename; the cwd-relative `node .claude/hooks/<name>.mjs` written by hand into `~/.claude/settings.json`; and an invocation whose stdout is discarded, which for the guard means its denial is. Matcher coverage is neither string equality nor "it's a regex": Claude Code 2.1.220 has **three** tiers, read out of the installed binary next to the literal `Invalid regex pattern in hook matcher:` — absent/empty/`*` is match-all, `/^[a-zA-Z0-9_|]+$/` is an exact `|`-separated **list** compared with `includes(tool)`, and only anything else is an unanchored regex. Tier 2 is why the regex-only reading was unsafe: `matcher: "as"` matches `Bash` as a regex but not as a list, so calling it covering left Bash with no guard at all. That is client behavior — re-check it against the binary when the pinned version moves. `scripts/install.test.mjs` has a case per form and per tier, plus two that pin idempotency.
